...
Utilisé par : compteur (démo), `HxTabs`, `HxTable` (tri + page dans l'URL, le serveur trie/pagine et le composant ne fait que rendre), `HxAccordion` (clé du panneau ouvert dans `?open=`), `HxWizard` (l'étape demandée dans l'URL, les valeurs déjà saisies re-postées à chaque action via des champs cachés — le bouton Retour POSTe aussi, avec `hx-include="closest form"` pour embarquer le jeton antiforgery et les champs sans déclencher la validation navigateur), sélection d'une suggestion `HxAutocomplete` (le fragment hôte se re-rend avec la valeur choisie), **multi-sélection** `HxDropdown` (chaque `HxDropdownItem` à `Selected` non nul devient un `menuitemcheckbox` ; le toggle re-rend le fragment hôte avec la liste des valeurs dans l'URL, et le panneau reste ouvert car le fragment hôte le re-rend inline via `PanelContent` — la sélection est aussi exposée en champs cachés `Name`/`Values` pour un formulaire englobant) et **multi-sélection** `HxAutocomplete` (`Values` rend les valeurs choisies en chips supprimables adossées à des champs cachés ; ceux-ci accompagnent chaque recherche via `hx-include`, ce qui permet à l'endpoint de suggestions d'exclure les valeurs déjà choisies et de propager la sélection dans les URLs de pick/suppression). *C'est le pattern par défaut pour tout composant à état.*
