...
| `HxDropdown` / `HxDropdownPanel` / `HxDropdownItem` | Dropdown panel fetched as a fragment; closes on backdrop click or when an endpoint raises the `hx-dropdown-close` event through `HX-Trigger`. Multi-select supported: items carry a `Selected` state (`menuitemcheckbox`) and toggle by re-rendering the host fragment, which keeps the panel open through `PanelContent` and exposes the selection as hidden inputs (`Name`/`Values`). |
...
| `HxAutocomplete` / `HxAutocompletePanel` / `HxAutocompleteOption` | Debounced input fetching a suggestions fragment into a panel; picking a suggestion re-renders the host fragment with the value set (`outerHTML` self-swap). Multi-select supported: `Values` renders removable chips backed by hidden inputs that accompany every search request, so the endpoint can exclude picked values and preserve the selection in pick/remove URLs. |
